               ...
               srslte::log_ref          log_h_)
{
  ...
  if (phy && rlc && log_h_) {
    ...
    args  = args_;
    printf("[ca-debug] scell_act_policy=%s, scell_act_rbs=%uK\n.", args.scell_act_policy.c_str(), args.scell_act_rbs); //qr-conf
    cells = cells_;
  ...
  if (scell_cc_idx <= 1 && scell_active) {
    ...
    if (tx_queue < args.scell_act_rbs * 1000) {
      ...
      if (scell_act_time.count(rnti)) {
        ...
        if (second_diff > 10 && ue_db.count(rnti)) {
          log_h->info("[ca-debug] active SCell rnti=0x%x, cc_idx=%u, rbs=%u, time_diff=%d\n", rnti, scell_cc_idx, tx_queue, second_diff);
          // printf("[ca-debug] active SCell rnti=0x%x, cc_idx=%u, rbs=%u, time_diff=%d\n", rnti, scell_cc_idx, tx_queue, second_diff);
    ...
    if (tx_queue > args.scell_act_rbs * 1000) {
      ...
      if (scell_deact_time.count(rnti)) {
        ...
        if (second_diff > 10 && ue_db.count(rnti)) {
          log_h->info("[ca-debug] inactive SCell rnti=0x%x, cc_idx=%u, rbs=%u, time_diff=%d\n", rnti, scell_cc_idx, tx_queue, second_diff);
          // printf("[ca-debug] inactive SCell rnti=0x%x, cc_idx=%u, rbs=%u, time_diff=%d\n", rnti, scell_cc_idx, tx_queue, second_diff);
